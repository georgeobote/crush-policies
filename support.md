# Support

_Last updated: 2026-01-20_

Need help with Crush? You’ve got options.

## Contact

- **General support:** `hello@crushso.com`
- **Privacy / data requests:** `privacy@crushso.com`

When you email, include the phone number on your account (and your school email if you verified) so we can find the right record faster.

## Quick links

- How Crush works: https://georgeobote.github.io/crush-policies/how-crush-works
- Privacy Policy: https://georgeobote.github.io/crush-policies/privacy-policy
- Terms of Service: https://georgeobote.github.io/crush-policies/terms-of-service

## Account deletion

You can delete your account in the app: **Settings → Safety & privacy → Delete Account**.

If you’re locked out, email `hello@crushso.com` from the address/number tied to your account and we’ll help you complete deletion.

## Common issues

- **Not getting an SMS code:** Double-check your number, signal, and that you can receive SMS. If it still won’t send, email us with your country + carrier.
- **`.edu` verification email not arriving:** Check spam/junk and search your inbox for “Crush”. Some school domains delay external mail; try again after a few minutes.
- **Crush Drops paused:** Drops require verified SMS + `.edu`, your gender + bucketed “looking for” set, and at least **3 photos** on your profile. Add what’s missing and drops will resume.
- **Notifications missing:** Confirm notifications are enabled in-app and in your phone’s system settings.

## Safety

- **Block/report:** You can block or report from profiles and chats. Reports go to our Trust & Safety queue for review.
- **Emergency:** If you’re in immediate danger, contact local emergency services first.
